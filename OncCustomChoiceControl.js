        ...
        return {
            ...
            properties: {
                ...
                inputData: {
                    type: 'string',
                    title: 'Input Data',
                    description: 'Provide the data (string) from the SharePoint column/api response, which will be displayed as options in the control',
                    defaultValue: ""
                },
                separator: {
                    type: 'string',
                ...
                outcome: {
                    type: 'string',
                    title: 'Outcome',
                    description: 'If set, the value will be overridden by plugin response.This will store selected values.',
                    isValueField: true,
                    standardProperties: {
                        visibility: false
                    },
...
.custom-choice-dropdown .dropdown-toggle {
    display: flex;
...
.custom-choice-dropdown .dropdown-content {
    position: absolute;
    top: 100%;
    left: 0;
    right: 0;
...
.custom-choice-dropdown.open .dropdown-content {
    display: block;
}
...
.dropdown-content input[type="checkbox"] {
  ...
  `;
    ...
    constructDropdownTemplate(items) {
        ...
        if (this.currentPageMode == 'New' || this.currentPageMode == 'Edit') {
            if (Array.isArray(items)) {
                ...
                this.message = html`<select class="form-control customchoice-control" @change=${e => this._propagateOutcomeChanges(e.target.value)} >
                              ${itemTemplates}
                            </select>
                        `
            }
        ...
        if (this.currentPageMode == 'New' || this.currentPageMode == 'Edit') {
            if (Array.isArray(items)) {
              ...
              ${i}
                ...
                this.message = html`                    
                     <div class="custom-choice-dropdown" @click=${this._toggleDropdown}>
            <div class="dropdown-toggle">
              <span>${this.outcome.length > 0 ? this.outcome.join(', ') : 'Select Options'}</span>
              <span>&#9660;</span>
            </div>
    ...
    _toggleDropdown() {
        const dropdown = this.shadowRoot.querySelector('.custom-choice-dropdown');
